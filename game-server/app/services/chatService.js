...
ChatService.prototype.add = function(uid, playerName, channelName) {
  ...
  var currentChanel = channelName + currentChanel;
  var channel = this.app.get('channelService').getChannel(currentChanel, false);
  
  if(!channel) {
  	 channel = this.app.get('channelService').createChannel(currentChanel);
  }
  else if(channel && this.numberInChanel[channelName] == 2){
    currentChanel += 1;
  	currentChanel = chanelName + currentChanel;
  	channel = this.app.get('channelService').createChannel(currentChanel);
  }
  
  if(!this.numberInChanel[channelName]) this.numberInChanel[channelName] = 0;
   channel.add(uid, sid);
   this.numberInChanel[currentChanel] += 1;
